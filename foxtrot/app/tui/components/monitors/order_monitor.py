...
from foxtrot.util.constants import Status, Direction, OrderType
...
class TUIOrderMonitor(TUIDataMonitor):
    ...
    async def _update_order_statistics(self, order_data: OrderData) -> None:
        ...
        """
        # Update total orders
        if order_data.vt_orderid not in self.active_orders and order_data.vt_orderid not in self.completed_orders:
    ...
    class AllOrdersCancelRequested(Message):
        ...
        def __init__(self, symbol_filter: Optional[str] = None) -> None:
