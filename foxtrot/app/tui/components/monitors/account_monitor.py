...
class TUIAccountMonitor(TUIDataMonitor):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        # Filtering options
        self.currency_filter: Optional[str] = None
        self.gateway_filter: Optional[str] = None
        self.min_balance_filter: Optional[float] = None
        ...
        try:
            ...
            if stats["total_balance"] > 0:
                self.risk_metrics["risk_utilization"] = (
                    (stats["total_frozen"] + stats["total_margin"]) / stats["total_balance"]
                )
            else:
    ...
    async def _check_risk_warnings(self, account_data: AccountData) -> None:
        ...
        if account_data.available < self.balance_warning_threshold:
            warnings.append(f"Low available balance: {TUIFormatter.format_currency(account_data.available)}")
        ...
        if hasattr(account_data, 'margin') and account_data.margin > 0:
            ...
            if margin_ratio > self.margin_warning_threshold:
                warnings.append(f"High margin usage: {TUIFormatter.format_percentage(margin_ratio)}")
    ...
    async def action_save_csv(self) -> None:
        ...
        try:
            ...
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                ...
                headers.extend([
                    "Equity", "Margin Ratio", "Risk Level", "Export Date", "Filter Applied",
                    "Total Accounts", "Total Equity", "Portfolio Risk"
                ])
                ...
                for row_index in range(self.data_table.row_count):
                    ...
                    if row_index == 0:
                        row_data.extend([
                            equity, margin_ratio, risk_level, export_time, filter_info,
