...
class TUIPositionMonitor(TUIDataMonitor):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.exchange_filter: Optional[Exchange] = None
        self.gateway_filter: Optional[str] = None
        self.min_pnl_filter: Optional[float] = None
        self.min_value_filter: Optional[float] = None
    ...
    async def _check_risk_warnings(self, position_data: PositionData) -> None:
        ...
        if position_data.pnl < self.pnl_warning_threshold:
            warnings.append(f"Large loss: {TUIFormatter.format_pnl(position_data.pnl)} on {position_data.symbol}")
        
        # Check for large position size
        position_value = abs(position_data.price * position_data.volume)
        if position_value > self.large_position_threshold:
            warnings.append(f"Large position: {TUIFormatter.format_currency(position_value)} in {position_data.symbol}")
    ...
    def get_portfolio_summary(self) -> Dict[str, Any]:
        ...
        stats["win_rate"] = (
            stats["winning_positions"] / max(stats["total_positions"], 1) * 100
        )
    ...
    def get_symbol_exposure(self, symbol: str) -> Dict[str, Any]:
        ...
        """
        symbol_positions = [
            pos for pos in self.row_data.values()
    ...
    def action_toggle_percentage_display(self) -> None:
        ...
        asyncio.create_task(self._add_system_message(
    ...
    async def action_save_csv(self) -> None:
        ...
        try:
            ...
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                ...
                headers.extend([
                    "Position Value", "Risk Level", "Export Date", "Filter Applied",
                    "Total Positions", "Portfolio P&L", "Win Rate"
                ])
                ...
                for row_index in range(self.data_table.row_count):
                    ...
                    risk_level = "MEDIUM"  # Would be calculated based on position size and volatility
                    
                    # Add metadata to first row only
                    if row_index == 0:
                        row_data.extend([
                            position_value, risk_level, export_time, filter_info,
...
def create_position_monitor(main_engine: MainEngine, event_engine: EventEngine) -> TUIPositionMonitor:
    """
