...
from textual.validation import Function, ValidationResult as TextualValidationResult
...
from foxtrot.util.object import OrderData, Direction, OrderType
from foxtrot.util.constants import Exchange

from ..integration.event_adapter import TUIEventMixin
from ..validation import (
    SymbolValidator, PriceValidator, VolumeValidator, 
    OrderTypeValidator, DirectionValidator, ExchangeValidator,
    FormValidator, ValidationResult, ValidationErrorCollector
)
from ..utils.formatters import format_currency, format_number
from ..utils.colors import get_color_for_value
...
class OrderPreviewPanel(Container):
    ...
    async def _calculate_preview(self):
        ...
        try:
            ...
            self._show_error(f"Preview calculation error: {e}")
    ...
    async def _get_available_funds(self) -> Optional[Decimal]:
    ...
    async def _update_preview_display(
        ...
        order_value: Decimal,
        commission: Decimal,
        available_funds: Optional[Decimal],
        position_impact: str
    ):
...
class MarketDataPanel(Container):
    ...
    async def _fetch_market_data(self):
        ...
        try:
            ...
            self._show_error(f"Market data error: {e}")
...
class TUITradingPanel(Container, TUIEventMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cancel_all_button: Optional[Button] = None
    ...
    async def _get_form_data(self) -> Dict[str, Any]:
        ...
        if self.price_input and self.price_input.value:
            try:
                data["price"] = Decimal(self.price_input.value)
            except:
                data["price"] = None
        
        if self.volume_input and self.volume_input.value:
            try:
                data["volume"] = Decimal(self.volume_input.value)
            except:
                data["volume"] = None
    ...
    async def _submit_order(self):
        ...
        try:
            ...
            await self._show_error(f"Order submission failed: {e}")
    ...
    async def _confirm_order(self, order_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self._show_error(f"Confirmation dialog error: {e}")
            return False
    ...
    async def _send_order_to_engine(self, order_data: Dict[str, Any]):
        ...
        try:
            ...
            await self._show_error(f"Failed to submit order: {e}")
    ...
    async def _cancel_all_orders(self):
        ...
        try:
            ...
            await self._show_error(f"Failed to show cancel confirmation: {e}")
    ...
    async def _execute_cancel_all(self):
        ...
        try:
            ...
            await self._show_error(f"Failed to cancel all orders: {e}")
