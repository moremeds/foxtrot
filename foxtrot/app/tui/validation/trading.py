        ...
        if self.contract_manager:
            try:
                ...
                secure_error = SecurityAwareErrorHandler.handle_exception(
                    e,
                    context="symbol_validation",
                    user_data={"symbol": symbol_str}
                )
                result.add_error(secure_error.user_message)
                return result

        result.value = symbol_str
        return result

    def _get_contract_info(self, symbol: str, exchange: str | None) -> ContractInfo | None:
        """
...
class VolumeValidator(FieldValidator):
    ...
    def _get_available_funds(self) -> Decimal | None:
        """
