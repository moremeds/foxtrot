...
class TestEventEngineRaceConditions:
    ...
    def test_engine_state_consistency(self):
        ...
        def state_checker():
            ...
            for _ in range(100):
                ...
                thread_alive = (
                    ...
                    if hasattr(self.engine._thread, "is_alive")
                    else False
                )
                timer_alive = (
                    self.engine._timer.is_alive()
                    if hasattr(self.engine._timer, "is_alive")
                    else False
                )
        ...
        for state in state_checks:
            if state.startswith("active_threads:False") or state.startswith(
                "inactive_threads:True"
            ):
