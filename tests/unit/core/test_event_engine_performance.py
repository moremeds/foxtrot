...
from foxtrot.core.event_engine import EVENT_TIMER, Event, EventEngine
...
class TestEventEnginePerformance:
    ...
    def test_event_processing_throughput(self):
        ...
        if metrics["memory_delta"] > 0:
            assert metrics["memory_delta"] < 500, (
                f"Excessive memory usage: {metrics['memory_delta']:.2f} MB"
            )
    ...
    def test_event_processing_latency(self):
        ...
        assert len(latencies) == num_events, (
            f"Only {len(latencies)}/{num_events} latencies measured"
        )
    ...
    def test_concurrent_throughput(self):
        ...
        assert len(processed_events) == total_events, (
            f"Only {len(processed_events)}/{total_events} events processed"
        )

        # Performance assertions
        assert events_per_second > 800, (
            f"Concurrent throughput too low: {events_per_second:.2f} events/sec"
        )
    ...
    def test_timer_precision_under_load(self):
        ...
        assert abs(avg_interval - expected_interval) < 0.01, (
            f"Timer drift too high: {avg_interval:.3f}s vs {expected_interval:.3f}s"
        )
...
class TestEventEngineStressTest:
    ...
    def test_sustained_load(self):
        ...
        assert events_per_second > 10, (
            f"Sustained throughput too low: {events_per_second:.2f} events/sec"
        )
        assert memory_growth_rate < 50, (
            f"Memory growth rate too high: {memory_growth_rate:.2f} MB/min"
        )
