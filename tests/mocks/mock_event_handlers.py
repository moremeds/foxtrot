...
from foxtrot.core.event_engine import Event, HandlerType
...
class MockEventHandler:
    ...
    def configure(self, fail_rate: float = 0.0, processing_delay: float = 0.0, enabled: bool = True):
...
class RecordingHandler(MockEventHandler):
    ...
    def get_latest_event(self, event_type: str = None) -> Optional[Event]:
...
class HandlerFactory:
    ...
    @staticmethod
    def create_load_testing_handler(name: str = None, cpu_intensive: bool = False) -> LoadTestingHandler:
...
class HandlerGroup:
    ...
    def remove_handler(self, key: str) -> Optional[MockEventHandler]:
        """Remove and return a handler from the group."""
        with self.lock:
            return self.handlers.pop(key, None)
    
    def get_handler(self, key: str) -> Optional[MockEventHandler]:
    ...
    def get_total_error_count(self) -> int:
        ...
        return sum(len(handler.errors) for handler in self.handlers.values() if hasattr(handler, 'errors'))
